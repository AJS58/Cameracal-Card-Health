...
/// Batch-check file headers for a list of paths.
/// Reads only ≤ 284 bytes per file (16 header + 12 tail + 256 mid-sample).
#[tauri::command]
pub fn check_files_integrity(paths: Vec<String>) -> Vec<IntegrityReport> {
...
fn check_integrity_one(path: &str) -> IntegrityReport {
    ...
    // Read sample at ~30% into file (256 bytes)
